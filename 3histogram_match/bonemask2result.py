...
def bonesuppress(root,xray_root,lung_root,save_root,resultsize,xraychanles,equal_hist=0):
    ...
    dia_bone_mask=morphology.dilation(bone_mask,np.ones([20,20]))
    ...
    if match_hist:
        ...
        dia_bone_mask=morphology.dilation(bone_mask.astype(np.bool),np.ones([3,3]))
    ...
    if blur_border:
        ...
        dia_bone_mask2blur=morphology.dilation(bone_mask.astype(np.bool),np.ones([10,10]))
        dia_bone_mask2blur=morphology.remove_small_holes(dia_bone_mask2blur,500*times)
        ero_bone_mask2blur=morphology.erosion(dia_bone_mask2blur,np.ones([20,20]))
...
if __name__=='__main__':
    bone_root=r'../2Pytorch-UNet-master_trainbonemask/bone'
    xray_root=r'./dataset'
    lung_root=r'../2Pytorch-UNet-master_trainbonemask/lung'
    save_root=r'./result'
